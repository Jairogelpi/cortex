...
Arquitectura: Sim( Phi(datos_frescos_independientes), Z_referencia_isomorfo )
Fixes:
    1. FRED: no usa parse_dates
    ...
    4. H1: token_tracker registra tokens reales de las 2 llamadas LLM
"""
import time
import json
import re
    ...
    def summary(self) -> str:
        ...
        if self.contradictions:
            ...
            for c in self.contradictions:
                lines.append(f"    - {c}")
        lines.append(f"  Razonamiento: {self.reasoning}")
        return "\n".join(lines)
...
class LambdaLayer:
    ...
    def __init__(self):
        ...
        logger.info(
            f"Capa Lambda inicializada: {self.model} "
    ...
    def validate(self, omega_hypothesis: OmegaHypothesis, phi_state: PhiState) -> LambdaValidation:
        ...
        if "error" not in fred_data:
            ...
            logger.info(f"Lambda: FRED unavailable: {fred_data.get('error','Unknown')}")
    ...
    def _get_fred_data(self) -> dict:
        results = {}
        for series_id, field_name in [("T10Y2Y","fred_t10y2y_spread"),("VIXCLS","fred_vix_close")]:
    ...
    def _fetch_fred_csv_raw(self, series_id: str) -> Optional[float]:
        try:
            ...
            response = requests.get(url, headers={"User-Agent":"CortexV2/1.0"},
            ...
            return float(df_clean.iloc[-1][df.columns[1]]) if not df_clean.empty else None
        except Exception as e:
    ...
    def _adjust_for_additional_signals(self, similarity, signals, isomorph_name, trading_signal):
        ...
        if isomorph_name in ("lorenz_attractor", "phase_transition"):
            if vix_change < -8.0:
                penalty += 0.30
                contradictions.append(f"VIX cayo {vix_change:.1f}pts en 5d: descompresion severa incompatible con caos activo")
            elif vix_change < -5.0:
                penalty += 0.18
                contradictions.append(f"VIX cayo {vix_change:.1f}pts en 5d: estres bajando, inconsistente con {isomorph_name}")
            elif vix_change < -3.0:
                penalty += 0.09
        ...
        if abs(penalty) > 0.01:
            logger.info(f"Lambda: penalizacion neta={penalty:+.3f} (sim {similarity:.4f} -> {sim_adj:.4f})")
...
CONTRADICCIONES DETECTADAS: {len(existing_contradictions)}
...
{{"reasoning": "analisis critico 3-4 frases con datos especificos", "contradictions_found": []}}"""
        ...
        try:
            ...
            # H1: registrar tokens reales de Lambda (2 llamadas LLM: phi interna + este reasoning)
            # La llamada interna de Phi (temperature=0.0) no usa LLM — es determinista
            # Solo esta llamada cuenta como token real de Lambda
    ...
    def _failure_protocol(self, hypothesis, phi_state, start_time):
        return LambdaValidation(
            ...
            z_fresh=[0.0]*8,
            z_reference=list(PHYSICAL_ISOMORPHS.get(
                hypothesis.best_isomorph,{}).get("Z", np.zeros(8)).tolist()),
